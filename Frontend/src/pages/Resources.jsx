...
const Resources = () => {
  ...
  const [form, setForm] = useState({ name: '', type: 'ROOM', location: '', capacity: 0, status: 'ACTIVE' });
  ...
  const handleEdit = (r) => {
    setForm({ name: r.name, type: r.type, location: r.location, capacity: r.capacity, status: r.status });
    setEditingId(r.id);
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setEditingId(null);
      setForm({ name: '', type: 'ROOM', location: '', capacity: 0, status: 'ACTIVE' });
      fetchResources();
    } catch (err) {
      alert('Failed to save resource: ' + (err.response?.data?.message || err.message));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div style={{ padding: '2rem 0' }}>
      ...
      {showForm && isAdmin && (
        <form onSubmit={handleSubmit} className="glass-panel" style={{ marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div style={{ gridColumn: '1 / -1' }}><h3 style={{ margin: 0 }}>{editingId ? 'Edit Resource' : 'Add New Resource'}</h3></div>
          <div>
            <label style={{ display: 'block', marginBottom: 6, color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Name</label>
            <input required type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text-primary)' }} />
          </div>
          <div>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: 6, color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Location</label>
            <input required type="text" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text-primary)' }} />
          </div>
          <div>
            <label style={{ display: 'block', marginBottom: 6, color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Capacity</label>
            <input required type="number" value={form.capacity} onChange={e => setForm({ ...form, capacity: Number(e.target.value) })} style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.05)', color: 'var(--text-primary)' }} />
          </div>
          <div>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={submitting}>{submitting ? 'Saving...' : 'Save Resource'}</button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.5rem' }}>
          {resources.map(resource => (
            <ResourceCard key={resource.id} resource={resource} isAdmin={isAdmin} onEdit={handleEdit} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </div>
