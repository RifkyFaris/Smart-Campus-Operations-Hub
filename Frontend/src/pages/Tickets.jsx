...
import {
  AlertTriangle, Plus, X, ArrowUpCircle, ArrowRightCircle, ArrowDownCircle,
  CheckCircle2, Clock, XCircle, RefreshCw, ImagePlus, Trash2,
} from 'lucide-react';
...
const Badge = ({ map, value }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: st.color,
      background: `${st.color}22`, padding: '4px 12px', borderRadius: 20, fontSize: '0.82rem', fontWeight: 600 }}>
...
const Lightbox = ({ url, onClose }) => (
  <div onClick={onClose} style={{
    ...
    <img src={url} alt="attachment" style={{ maxWidth: '90vw', maxHeight: '90vh',
      borderRadius: 12, boxShadow: '0 0 60px rgba(0,0,0,0.8)' }} onClick={e => e.stopPropagation()} />
    <button onClick={onClose} style={{ position: 'absolute', top: 20, right: 24, background: 'none',
      border: 'none', color: '#fff', cursor: 'pointer', fontSize: '1.5rem' }}>✕</button>
  </div>
);
    ...
    <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden', padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => onDelete && onDelete(ticket.id)}
...
const Tickets = () => {
  ...
  return (
    <div style={{ padding: '2rem 0' }}>
      ...
      {error && (
        <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid var(--status-warning)',
          color: 'var(--status-warning)', padding: '1rem', borderRadius: 8, marginBottom: '1.5rem' }}>
          {error}
        </div>
      )}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="glass-panel"
          style={{ marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={{ display: 'block', marginBottom: 8, color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
              Photos <span style={{ color: 'var(--text-secondary)', fontWeight: 400 }}>(optional · up to 5 images)</span>
            </label>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 10 }}>
                {previews.map((src, i) => (
                  <div key={i} style={{ position: 'relative' }}>
                    <img src={src} alt={`preview-${i}`} style={{ width: 88, height: 88, objectFit: 'cover',
            ...
            {photos.length < 5 && (
              <div
                ...
                style={{
                  border: '2px dashed var(--glass-border)', borderRadius: 10, padding: '18px 24px',
                  display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer',
                  color: 'var(--text-secondary)', fontSize: '0.88rem', transition: 'border-color 0.2s, background 0.2s',
                }}
              ...
              >
                <ImagePlus size={22} style={{ color: '#6366f1', flexShrink: 0 }} />
                <span>Click to add photos <span style={{ opacity: 0.6 }}>({photos.length}/5 selected)</span></span>
              </div>
            )}
            <input
              ref={fileInputRef}
